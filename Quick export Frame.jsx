...
myButton.onClick = function () {
  ...
  if (exportInfo) {
    ...
    var batCommand =
      'cmd.exe /k C:\\Users\\dalid\\OneDrive\\Documents\\Scripts\\CompressPNG.bat "' +
      exportInfo.windowsPath +
      '"';
      alert(batCommand);
    system.callSystem(batCommand);
  }
};
...
function generateIcons(subDir, currentCompName) {
  ...
  // First Icon 270x270
  var tempComp1 = comp.duplicate();
  tempComp1.name = "Temp_270x270";
  tempComp1.width = 270;
  tempComp1.height = 270;
  var firstIconPath = subDir.fsName + "/" + currentCompName + " Icon 270.png";
  tempComp1.saveFrameToPng(comp.time, File(firstIconPath));

  // Create Second Icon 282x210 and add Temp_270x270 as a layer
  var tempComp2 = app.project.items.addComp(
    "Temp_282x210",
    282,
    210,
    1,
  ...
  var layer = tempComp2.layers.add(tempComp1);

  // Calculate scale factor
  var scaleFactor = 210 / 270;
  layer.scale.setValue([scaleFactor * 100, scaleFactor * 100]);

  var secondIconPath = subDir.fsName + "/" + currentCompName + " Icon 282.png";
  tempComp2.saveFrameToPng(comp.time, File(secondIconPath));

  // Delete temporary compositions
  tempComp1.remove();
  tempComp2.remove();
}
